...
def check_create_session(request, *args, **kwargs):        
    ...
    new_session_count = request.GET.get('new_session_count',None)
    ...
    session_key = '' 
    activated_session_key = request.COOKIES.get('activatedsessionid',None)
    try:
        ...
        if sitequeuesession is None or session_count == 0:
                ...
                if script_exempt_key == settings.SCRIPT_EXEMPT_KEY:
                    ...
                    for blocked_script in BLOCKED_SCRIPTING:
                        if blocked_script in browser_agent:
                            response = HttpResponse(json.dumps({"status:": "Waiting", 'message': "Request Denied", "bs": True}), content_type='application/json', status=403)
                            return response
                        if crawleruseragents.is_crawler(browser_agent):
                            response = HttpResponse(json.dumps({"status:": "Waiting", 'message': "Request Denied", "cr" : True}), content_type='application/json', status=403)
                            return response                          
            ...
            if activated_session_id:
                sitesession['activated_session_id'] = activated_session_id
    ...
    if sitesession:     
        if "activated_session_id" in sitesession: 
            activated_session_id = sitesession["activated_session_id"]            
...
#        print({'url':active_session_url, 'queueurl': reverse('site-queue-page'),'session': memory_session['sitequeuesession'], 'idle_seconds':idle_seconds,'expiry': sitesession["expiry"]})#, 'idle': sitesession["idle"],'status': sitesession["status"],'total_active_session': total_active_session})#, 'total_waiting_session': total_waiting_session,'expiry_seconds': expiry_seconds,'session_key': session_key, 'queue_position' : queue_position ,'wait_time' : wait_time ,'waiting_queue_enabled': waiting_queue_enabled, 'wq': env('WAITING_QUEUE_ENABLED','False'), 'time_left_enabled': time_left_enabled, 'browser_inactivity_timeout': browser_inactivity_timeout, 'browser_inactivity_redirect': browser_inactivity_redirect, 'browser_inactivity_enabled': browser_inactivity_enabled,'custom_message': custom_message,'queue_name': queue_name, 'more_info_link' : more_info_link, 'show_queue_position': show_queue_position, 'max_queue_session_limit': max_queue_session_limit, 'max_queue_url_redirect': max_queue_url_redirect,'queue_inactivity_url': queue_inactivity_url, 'queue_waiting_room_url': queue_waiting_room_url })
        ...
        response.set_cookie('sitequeuesession', session_key, max_age=3600, samesite=None, domain=QUEUE_DOMAIN)   
        if sitesession:     
            if "activated_session_id" in sitesession:            
                response.set_cookie('activatedsessionid', sitesession["activated_session_id"], max_age=3600, samesite=None, domain=QUEUE_DOMAIN)
        return response
    else:
        ...
        response["Access-Control-Allow-Headers"] = "*"
        response["X-Frame-Options"] = "allow-from *"
        response.set_cookie('sitequeuesession', session_key, max_age=3600, domain=QUEUE_DOMAIN)
             
        if sitesession:
            if "activated_session_id" in sitesession:            
                response.set_cookie('activatedsessionid', sitesession["activated_session_id"], max_age=3600, domain=QUEUE_DOMAIN)        
        return response
