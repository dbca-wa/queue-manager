        ...
        if (sitequeuemanager.var.waiting_queue_enabled == true) {
            if (sitequeuemanager.var.browser_inactivity_enabled == true) {
                if (sitequeuemanager.var.browser_inactivity_time > sitequeuemanager.var.browser_inactivity_timeout) {
                    var pageheight = $(document).height();
                    if ($("#queue-inactivity").length == 0) {
                        $('html').prepend("<div id='queue-inactivity' style='width: 100%; position: absolute; z-index: 10; height: " + pageheight + "px'><div style='width: 100%; height: " + pageheight + "px;  background-image: url(" + '"' + sitequeuemanager.var.url + "/static/img/django_queue_manager/bg_tran_black.png" + '"' + "'  ><BR><BR><div class='qm-box'><h2>Are you still there?</h2><br><br>If you don't tap or click 'Yes' before the countdown hits zero you'll have to start over again with a new session.<br><br><br><button class='qm-button qm-blue' onclick='sitequeuemanager.inactivityConfirm();'>Yes</button>&nbsp;<button class='qm-button qm-red' id='qm-countdown'>30</button></div></div></div>");
